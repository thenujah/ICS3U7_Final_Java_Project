...
class Player {
	...
	public Player() {
		...
		collider = new Collider(rect.getX(), rect.getY(), 32, 32);
	...
	/* Collisions not working since it checks if any of the corners are inside the 
	other collider, but it doesn't check if it's on the edge and if it is, it's not colliding.
	Possible fixes:
		- Add midpoints to the sides and check if the collider contains those.
	*/
	...
	public void render(Graphics2D g) {
		...
		g.drawRect(collider.rect.getX(), collider.rect.getY(), collider.rect.getWidth(), collider.rect.getHeight());
	}
